...
```{r 1.01_dataloading}
getwd()
d.share <- read.csv(("../Data/Cleaned/inf_fac_share.csv"))
d.count <- read.csv(("../Data/Cleaned/inf_fac_count.csv")) 
```
...
All estimates can be plottes against each other to recognize similarities between GA, HTA and FNT:
...
On the other hand, it is the other way round with regard to the fare network tickets. The greater the proportion of GA or half-fare cards, the smaller the proportion of group season tickets tends to be. *literature ?*
